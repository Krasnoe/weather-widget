...
const api_key = 'b230336864c439037146ceca79b26433';
const city_name = ref([]);
...
const getStartCity = (() => {
  if (!city_name.value.length) {
    navigator.geolocation.getCurrentPosition((pos) => {
      lat.value = +pos.coords.latitude.toFixed(2);
      lon.value = +pos.coords.longitude.toFixed(2);
      
      if (lat.value && lon.value) {
        axios.get(`https://api.openweathermap.org/data/2.5/weather?lat=${lat.value}&lon=${lon.value}&appid=${api_key}`)
        .then((res) => {
          city_name.value.push(res.data.name);
